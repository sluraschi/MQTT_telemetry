...
class Segment:
    def __init__(self, payload_type, segment_id, segment_date, segment_microsec, segment_rate, flag1, flag2, flag3, flag4, payload):
        ...
        self.segment_date = datetime.strptime(str(segment_date), '%Y%m%d%H%M%S').strftime('%m/%d/%Y %H:%M:%S')
        ...
        self.flag3 = flag3
        self.flag4 = flag4
        self.payload = self.build_payload(segment_date, payload, payload_type)

    @staticmethod
    def build_payload(seg_date, payload, payload_type):
        if payload_type == PayloadTypes.seismic.name:
